...
def calc_accuracy(tinyface_test, probe, gallery, do_norm=True):
    ...
    acc_list = []
    cor_list = []
    for rank in [1, 5, 10, 50]:  # Updated ranks
        correct = 0
        cor = 0
    ...
    print("Accuracy for Top 1: {:.2f}%".format(acc_list[0]))
    print("Accuracy for Top 5: {:.2f}%".format(acc_list[1]))
    print("Accuracy for Top 10: {:.2f}%".format(acc_list[2]))
    print("Accuracy for Top 50: {:.2f}%".format(acc_list[3]))
    pd.DataFrame({'rank':[1, 5, 10, 50], 'values':acc_list}).to_csv(os.path.join(args.save_dir, 'tinyface_result.csv'), index=False)
    print("Accuracy for our data")
    print("Accuracy for Top 1: {:.2f}%".format(cor_list[0]))
    print("Accuracy for Top 5: {:.2f}%".format(cor_list[1]))
    print("Accuracy for Top 10: {:.2f}%".format(cor_list[2]))
    print("Accuracy for Top 50: {:.2f}%".format(cor_list[3]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tinyface_dir', default='/home/kush/VISION/tinyface/Testing_Set')
    parser.add_argument('--gpus', default='1', type=str)
    parser.add_argument('--batch_size', default=256, type=int, help='')
    parser.add_argument('--mode', type=str, default='cbam', help='attention type')
    parser.add_argument('--save_dir', type=str, default='result/')
    parser.add_argument('--checkpoint_path', type=str, default='/home/kush/VISION/student_241/last_net.ckpt', help='scale size')
    ...
    print("Total latency: {:.4f} seconds".format(total_latency))  # Print total latency
